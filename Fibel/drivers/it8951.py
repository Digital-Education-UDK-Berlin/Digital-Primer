...
class IT8951(DisplayDriver):
    ...
    def init(self,screen="front",rotate=ROTATE_270):
        if screen=="back":
            self.SCREEN="back"
            self.CS_PIN=23
            self.BUSY_PIN=22
        ...
        sys.stderr.write("width = %d" % self.width)
        sys.stderr.write("height = %d" % self.height)
        sys.stderr.write("img_addr = %08x" % self.img_addr)
        sys.stderr.write("firmware = %s" % firmware_version)
        sys.stderr.write("lut = %s" % lut_version)
        ...
        if self.VCOM != self.get_vcom():
            ...
            sys.stderr.write("VCOM = -%.02fV" % (self.get_vcom() / 1000.0))

        # Initialize the display with a blank image.
        self.wait_for_ready()
        #image = Image.new("1", (self.width, self.height), 0x255)
    ...
    def display_buffer_area(self, x, y, w, h, display_mode, buff_addr):
        self.write_command(self.CMD_BUFFER_AREA)
        self.write_data_half_word(x)
        self.write_data_half_word(y)
        self.write_data_half_word(w)
        self.write_data_half_word(h)
    ...
    def load_image(self, x, y, image, update_mode_override=None, img_addr=None):
        width = image.size[0]
        height = image.size[1]
        ...
        self.write_register(
                self.REG_MEMORY_CONV_LISAR + 2, (img_addr >> 16) & 0xFFFF)
        self.write_register(self.REG_MEMORY_CONV_LISAR, img_addr & 0xFFFF)
